#!/usr/bin/env python3
"""Cron runner for Navajo foreclosure pipeline (every 15 minutes)."""
...
LOCKFILE = TMP_DIR / "navajo_cron.lock"
lock_fd = open(LOCKFILE, "w")


def _log(msg: str) -> None:
    with open(LOG_DIR / "navajo_cron.log", "a", encoding="utf-8") as f:
...
try:
    ...
    latest_csv = OUT_DIR / "navajo_latest.csv"
    latest_json = OUT_DIR / "navajo_latest.json"
    if csv_path.exists():
        shutil.copyfile(csv_path, latest_csv)
        shutil.copyfile(csv_path, OUT_DIR / f"navajo_daily_{date.today():%Y%m%d}.csv")
    if json_path.exists():
