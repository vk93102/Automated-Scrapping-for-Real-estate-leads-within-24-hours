#!/usr/bin/env python3
"""Navajo County, AZ pipeline built on shared county-recorder flow."""
...
def run_navajo_pipeline(*args, **kwargs):
    ...
    if not csv_path_s and not json_path_s:
        if isinstance(res.get("summary"), dict):
            res["summary"]["county"] = "Navajo County, AZ"
        return res
    ...
    if ts:
        new_csv = OUTPUT_DIR / f"navajo_leads_{ts}.csv"
        new_json = OUTPUT_DIR / f"navajo_leads_{ts}.json"
    ...
    if json_out_s:
        ...
        try:
            ...
            if isinstance(payload.get("meta"), dict):
                payload["meta"]["county"] = "Navajo County, AZ"
