#!/usr/bin/env python3
"""Coconino interval runner: fetch last N days and upsert records to DB."""
...
def _log(msg: str) -> None:
    ...
    with (log_dir / "conino_interval.log").open("a", encoding="utf-8") as f:
        f.write(line + "\n")
...
def _ensure_schema(conn: psycopg.Connection) -> None:
    with conn.cursor() as cur:
        cur.execute(
            """
            create table if not exists conino_leads (
        ...
        cur.execute(
            """
            create table if not exists conino_pipeline_runs (
              id              bigserial primary key,
...
def _upsert_records(conn: psycopg.Connection, records: list[dict], run_date: date) -> tuple[int, int, int]:
    ...
    with conn.cursor() as cur:
        for r in records:
            ...
            groq_model = ""
            groq_error = str(r.get("groqError", "") or "")
            ocr_method = str(r.get("ocrMethod", "") or "")
            ocr_chars = 0
            if isinstance(r.get("documentAnalysis"), dict):
                da = r["documentAnalysis"]
                used_groq = bool(da.get("usedGroq", used_groq))
                groq_model = str((da.get("groqAnalysis") or {}).get("model", "") or "")
                groq_error = str(da.get("groqError", groq_error) or "")
                ocr_method = str(da.get("ocrMethod", ocr_method) or "")
                ocr_chars = int(da.get("ocrTextLength") or 0)
            if used_groq:
                llm_used += 1
            ...
            payload = {
                ...
                "image_urls": r.get("documentUrl", ""),
                "ocr_method": ocr_method,
                "ocr_chars": ocr_chars,
                "used_groq": used_groq,
                "groq_model": groq_model,
                "groq_error": groq_error,
                "analysis_error": r.get("documentAnalysisError", ""),
                "run_date": run_date,
                "raw_record": psycopg.types.json.Jsonb(r),
            }
            cur.execute(
                """
                insert into conino_leads (
...
def _run_once(doc_types: list[str], lookback_days: int, page_limit: int | None, ocr_limit: int, strict_llm: bool) -> tuple[int, int, int, int]:
    ...
    if strict_llm:
        def _used_llm(rec: dict) -> bool:
            if bool(rec.get("usedGroq", False)):
                return True
            da = rec.get("documentAnalysis") or {}
            if isinstance(da, dict):
                return bool(da.get("usedGroq", False))
            return False

        missing = [str(r.get("documentId", "") or "") for r in records if not _used_llm(r)]
        if missing:
    ...
    with _connect_db(db_url) as conn:
        ...
        with conn.cursor() as cur:
            cur.execute(
                """
                insert into conino_pipeline_runs
...
def main() -> None:
    ...
    p = argparse.ArgumentParser(description="Run Coconino pipeline on interval and upsert into DB")
    p.add_argument("--interval-minutes", type=float, default=720.0)
    p.add_argument("--lookback-days", type=int, default=7)
    p.add_argument("--pages", type=int, default=0, help="0 means all available pages")
    ...
    _log(
        f"starting conino interval runner interval_minutes={args.interval_minutes} "
        f"lookback_days={args.lookback_days} once={args.once} ocr_limit={args.ocr_limit}"
    )

    page_limit = None if args.pages == 0 else max(1, int(args.pages))
