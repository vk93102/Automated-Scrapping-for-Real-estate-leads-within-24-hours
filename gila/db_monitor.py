...
Usage examples:
  python gila/db_monitor.py --days 7
  python gila/db_monitor.py --days 7 --watch --interval 30
...
def print_status(*, days: int, run_limit: int, doc_type_limit: int, show_leads: int) -> None:
    ...
    with _connect_db(db_url) as conn:
        with conn.cursor() as cur:
            total_leads = _fetch_one(cur, "select count(*) from public.gila_leads")
            total_runs = _fetch_one(cur, "select count(*) from public.gila_pipeline_runs")

            print(f"asof={datetime.now():%Y-%m-%d %H:%M:%S}")
            print(f"gila_leads_total={total_leads} gila_pipeline_runs_total={total_runs}")

            print("\nleads_by_run_date:")
            cur.execute(
                """
                select run_date, count(*)
                from public.gila_leads
            ...
            cur.execute(
                ...
                from public.gila_pipeline_runs
                order by id desc
                limit %s;
                """,
                (max(1, int(run_limit)),),
...
def main() -> None:
    p = argparse.ArgumentParser(description="Monitor Gila Supabase tables")
    p.add_argument("--days", type=int, default=7, help="Window size for grouped stats")
    p.add_argument("--runs", type=int, default=8, help="How many recent runs to print")
    p.add_argument("--doc-types", type=int, default=15, help="How many document types to list")
