#!/usr/bin/env python3
"""Cron runner for La Paz foreclosure pipeline (every 15 minutes)."""
...
LAPAZ_DIR = Path(__file__).resolve().parent
ROOT_DIR = LAPAZ_DIR.parent
...
OUT_DIR = LAPAZ_DIR / "output"

for d in (LOG_DIR, TMP_DIR, OUT_DIR):
    d.mkdir(parents=True, exist_ok=True)

LOCKFILE = TMP_DIR / "lapaz_cron.lock"
lock_fd = open(LOCKFILE, "w")


def _log(msg: str) -> None:
    line = f"[{datetime.now():%Y-%m-%d %H:%M:%S}] {msg}\n"
    with open(LOG_DIR / "lapaz_cron.log", "a", encoding="utf-8") as f:
        f.write(line)
...
try:
    ...
    latest_csv = OUT_DIR / "lapaz_latest.csv"
    latest_json = OUT_DIR / "lapaz_latest.json"
    if csv_path.exists():
    ...
    daily_csv = OUT_DIR / f"lapaz_daily_{date.today():%Y%m%d}.csv"
    if csv_path.exists():
