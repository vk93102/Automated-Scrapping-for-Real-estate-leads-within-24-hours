#!/usr/bin/env python3
"""Cron runner for Santa Cruz foreclosure pipeline (every 15 minutes)."""
...
LOCKFILE = TMP_DIR / "santacruz_cron.lock"
lock_fd = open(LOCKFILE, "w")


def _log(msg: str) -> None:
    line = f"[{datetime.now():%Y-%m-%d %H:%M:%S}] {msg}\n"
    with open(LOG_DIR / "santacruz_cron.log", "a", encoding="utf-8") as f:
        f.write(line)
...
try:
    ...
    latest_csv = OUT_DIR / "santacruz_latest.csv"
    latest_json = OUT_DIR / "santacruz_latest.json"
    if csv_path.exists():
        shutil.copyfile(csv_path, latest_csv)
    if json_path.exists():
        shutil.copyfile(json_path, latest_json)

    daily_csv = OUT_DIR / f"santacruz_daily_{date.today():%Y%m%d}.csv"
    if csv_path.exists():
