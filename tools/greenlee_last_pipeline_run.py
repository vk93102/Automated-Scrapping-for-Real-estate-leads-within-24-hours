...
def main() -> int:
    ...
    with psycopg.connect(db_url, connect_timeout=12) as conn:
        with conn.cursor() as cur:
            cur.execute("select " + ",".join(cols) + " from greenlee_pipeline_runs order by id desc limit 1;")
            row = cur.fetchone()

    if not row:
        print("no greenlee_pipeline_runs rows")
        return 0

    print("latest_greenlee_pipeline_runs")
