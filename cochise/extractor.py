#!/usr/bin/env python3
"""Cochise County, AZ pipeline built on shared county-recorder flow."""
...
_base.COUNTY_LABEL = "COCHISE"
_base.COUNTY_DISPLAY = "Cochise"
...
def run_cochise_pipeline(*args, **kwargs):
    ...
    if ts:
        new_csv = OUTPUT_DIR / f"cochise_leads_{ts}.csv"
        new_json = OUTPUT_DIR / f"cochise_leads_{ts}.json"
        if csv_path.exists():
    ...
    if isinstance(res.get("summary"), dict):
        res["summary"]["county"] = "Cochise County, AZ"
    ...
    if json_out.exists():
        try:
            ...
            if isinstance(payload.get("meta"), dict):
                payload["meta"]["county"] = "Cochise County, AZ"
...
def playwright_collect_results(
    ...
    with sync_playwright() as pw:
        ...
        for dt in doc_types:
            if verbose:
                print(f"[GREENLEE] Searching doc type: {dt}")
